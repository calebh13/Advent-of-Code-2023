...
#include <ctype.h>
#include <limits.h>
#define NUM_SEEDS 20 // 20 for input.txt
#define NEW_LINE 1
#define SAME_LINE 0
#define NUM_MAPS 7

// note: for this to work properly you must insert two newlines at the very end of input.txt due to how the file is parsed; it just made it easier
...
unsigned int findNextDifferentSeed(unsigned int seed, unsigned int seedMax, Map maps[NUM_MAPS][50]);

int main(void)
{
	FILE* fp = fopen("input.txt", "r");
	Map maps[NUM_MAPS][50] = { { 0 } }; // nothing has more than 50 maps, and I didn't want to deal with dynamic memory, so this works
	unsigned int seeds[NUM_SEEDS] = { 0 };
	fillSeeds(fp, seeds);
	...
	fillMaps(fp, maps);
	unsigned int curLocation = UINT_MAX, minLocation = UINT_MAX, seedMax = 0, numSeedsChecked = 0;

	for (int i = 0; i < NUM_SEEDS; i += 2)
	{
		printf("Checking seed range %d ...\n", i);
		seedMax = seeds[i] + seeds[i + 1];
		for (unsigned int j = seeds[i]; j < seedMax; j++)
		{
			//printf("FIRST LOCATION TO FIND: SEED %u\n", j);
			curLocation = findLocation(j, maps);
			numSeedsChecked++;
			printf("Seed %u mapped to location %u.\n", j, curLocation);
			j = findNextDifferentSeed(j, seedMax, maps);
	...
	printf("Number of seeds checked: %u\n", numSeedsChecked);
	return 0;
}
...
unsigned int findLocation(unsigned int seed, Map maps[NUM_MAPS][50])
{
	...
	{
		prevSeed = seed;
		for (j = 0; maps[i][j].range != 0; j++) // range is never 0, so once we reach a 0 range, we know we are past all good values
		{
...
// Finds the VERY next seed within the range of seeds that gets a different SOIL value
// This is essentially what makes it not check 1 billion seeds, because due to the large ranges in part 2
// we could check 1 million seeds that all are within the same source range for soil.
unsigned int findNextDifferentSeed(unsigned int seed, unsigned int seedMax, Map maps[NUM_MAPS][50])
{
	unsigned int source = 0, range = 0;
	for (int j = 0; maps[0][j].range != 0; j++) // Go through all seed->soil maps
	{
		source = maps[0][j].source, range = maps[0][j].range;
		if ((seed >= source) && (seed < source + range)) // Seed is within this range, so we found the right one
		{
			printf("%u has been skipped to %u\n", seed, source + range - 1);
			return source+range - 1; // Increment
		}
	}
	printf("Seed %u not found in map. Returning self\n", seed);
	return seedMax - 1;
}
